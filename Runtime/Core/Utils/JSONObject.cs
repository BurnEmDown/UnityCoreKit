	...
	public class JSONObject {
		...
		public static JSONObject Create() {
			...
			while(result == null && releaseQueue.Count > 0) {
				...
				if(result == null)
					Logger.Log("wtf " + releaseQueue.Count);
				else if(result.list != null)
					Logger.Log("wtflist " + result.list.Count);
	#endif
			}
			if(result != null)
				return result;
		...
		void Parse(string str, int maxDepth = -2, bool storeExcessLevels = false, bool strict = false) {
			if(!string.IsNullOrEmpty(str)) {
				...
				if(strict) {
					if(str[0] != '[' && str[0] != '{') {
						type = Type.NULL;
						Logger.Log("Improper (strict) JSON formatting.  First character must be [ or {");
						return;
					}
				}
				if(str.Length > 0) {
						...
						switch(str[offset]) {
							...
							default:
								try {
									...
									type = Type.NULL;
									Logger.Log("improper JSON formatting:" + str);
								}
								return;
						}
						string propName = "";
		...
		static void MergeRecur(JSONObject left, JSONObject right) {
			if(left.type == Type.NULL)
				...
				if(right.Count > left.Count) {
					Logger.Log("Cannot merge arrays when right object has more elements");
					return;
				}
				for(int i = 0; i < right.list.Count; i++) {
			...
			if(type == Type.OBJECT) {
				...
				for(int i = 0; i < list.Count; i++) {
					...
					switch(val.type) {
						...
						default:
							Logger.Log("Omitting object: " + keys[i] + " in dictionary conversion");
							break;
					}
				}
				return result;
			}
			Logger.Log("Tried to turn non-Object JSONObject into a dictionary");
			return null;
		}
